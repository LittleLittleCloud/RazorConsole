...
- Query all completed github Issues/PRs under `https://github.com/LittleLittleCloud/RazorConsole`'s `v{VersionPrefix}` milestone. They would be added to that milestone when its completed/PR is merged, so you can use this approach to collect all changes included in the current release.
