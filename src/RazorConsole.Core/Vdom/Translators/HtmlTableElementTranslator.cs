...
public sealed partial class VdomSpectreTranslator
{
    internal sealed class TableElementTranslator : IVdomElementTranslator
    {
        ...
        public bool TryTranslate(VNode node, TranslationContext context, out IRenderable? renderable)
        {
            ...
            table.Expand = !string.Equals(GetAttribute(node, "data-expand"), "false", StringComparison.OrdinalIgnoreCase);
            ...
            var title = GetAttribute(node, "data-title");
            if (!string.IsNullOrWhiteSpace(title))
            {
                table.Title = new TableTitle(Markup.Escape(title));
            }

            var border = ResolveBorder(GetAttribute(node, "data-border"));
            table.Border = border;

            if (TryGetBoolAttribute(node, "data-show-headers", out var showHeaders))
            {
        ...
        private static bool TryBuildRow(VNode rowNode, TranslationContext context, out RowData rowData)
        {
            ...
            foreach (var child in rowNode.Children)
            {
                ...
                if (!TryConvertChildrenToRenderables(child.Children, context, out var renderables))
                {
                    rowData = default!;
                    return false;
                }

                var content = ComposeChildContent(renderables);


                var alignmentAttribute = GetAttribute(child, "data-align");
