...
<<<<<<< TODO: необъединенное изменение из проекта "RazorConsole.Core (net10.0)", До:
...
=======
        ...
        {
>>>>>>> После
        services.AddSingleton<Core.Vdom.VdomSpectreTranslator>(sp =>
        {
...
=======
            var translators = sp.GetServices<IVdomElementTranslator>()
                .OrderBy(t => t.Priority)
>>>>>>> После
            var translators = sp.GetServices<Core.Vdom.IVdomElementTranslator>()
                .OrderBy(t => t.Priority)
                .ToList();
...
=======
        var translator = serviceProvider.GetRequiredService<VdomSpectreTranslator>();
        return new ConsoleRenderer(serviceProvider, NullLoggerFactory.Instance, translator);
>>>>>>> После
        var translator = serviceProvider.GetRequiredService<Core.Vdom.VdomSpectreTranslator>();
        return new ConsoleRenderer(serviceProvider, NullLoggerFactory.Instance, translator);
    }
